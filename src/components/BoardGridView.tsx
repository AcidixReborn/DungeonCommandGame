...
/**
 * BoardGridView - Isolated board grid rendering component
 *
 * Purpose: Renders only the 64 board tiles. Memoized to prevent re-renders
 * when GameBoard state changes but board data hasn't changed.
 *
 * This is the KEY component for eliminating flashing:
 * - When panel tabs change, GameBoard re-renders due to renderCounter
 * - But BoardGridView checks if board data actually changed
 * - If not, it skips re-rendering the 64 tiles
 * - Result: No flash!
 */
function BoardGridView({
  gameState,
  // Creature position map
  creaturePositionMap,
...
// Don't memoize BoardGridView itself - let it re-render
// The optimization happens at the BoardTile level
// BoardTile has extensive memoization that prevents unnecessary re-renders
