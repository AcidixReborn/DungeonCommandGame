  ...
  creatureAbilityStats: {
    ...
    cormyr: {
      ...
      explosive_bolts: {
        ...
        splashDamageDealt: 0,
        targetsHit: 0,
        creatures: ['Half-Orc Thug'],
      },
      arcane_portal: {
        name: 'ARCANE PORTAL',
        timesTriggered: 0,
        portalDeployments: 0,
        creatures: ['War Wizard'],
      },
      flying_cormyr: {
        name: 'FLYING',
        timesTriggered: 0,
        terrainIgnored: 0,
        creatures: ['Copper Dragon'],
      },
      acid_breath: {
        name: 'ACID BREATH',
        timesTriggered: 0,
    ...
    gruumsh: {
      ...
      draw_order: {
        ...
        timesTriggered: 0,
        cardsDrawn: 0,
        creatures: ['Orc Cleric of Gruumsh'],
      },
      morale_boost: {
        name: 'MORALE BOOST',
        timesTriggered: 0,
    ...
    undeath: {
      ...
      life_drain: {
        ...
        timesTriggered: 0,
        hpHealed: 0,
        creatures: ['Vampire Stalker'],
      },
      aura_damage: {
        name: 'AURA DAMAGE',
        timesTriggered: 0,
        damageDealt: 0,
        creatures: ['Disciple of Kyuss'],
      },
      swirl: {
        name: 'SWIRL',
        timesTriggered: 0,
        splashDamageDealt: 0,
        creatures: ['Skeletal Tomb Guardian'],
      },
      rider_undeath: {
        name: 'RIDER',
        timesTriggered: 0,
        skeletonsDeployed: 0,
        creatures: ['Skeletal Lancer'],
      },
      undead_deployment: {
        name: 'UNDEAD DEPLOYMENT',
        timesTriggered: 0,
        undeadDeployed: 0,
        creatures: ['Lich Necromancer'],
      },
      phasing: {
        name: 'PHASING',
        timesTriggered: 0,
        wallsIgnored: 0,
        creatures: ['Hypnotic Spirit'],
      },
      insubstantial: {
        name: 'INSUBSTANTIAL',
        timesTriggered: 0,
        damagePreventedCompletely: 0,
        creatures: ['Hypnotic Spirit'],
      },
      flying_undeath: {
        name: 'FLYING',
    ...
    goblins: {
      opportunist: {
        ...
        timesTriggered: 0,
        bonusDamageDealt: 0,
        creatures: ['Goblin Cutter'],
      },
      tap_on_hit: {
        name: 'TAP ON HIT',
        timesTriggered: 0,
        creaturesTapped: 0,
        creatures: ['Wolf', 'Horned Devil'],
      },
      flanking_goblin: {
        name: 'FLANKING',
        timesTriggered: 0,
        bonusDamageDealt: 0,
        creatures: ['Goblin Champion'],
      },
      magic_circle_aura: {
        name: 'MAGIC CIRCLE AURA',
        timesTriggered: 0,
        damageBlocked: 0,
        creatures: ['Hobgoblin Sorcerer'],
      },
      untap_on_kill_goblin: {
        name: 'UNTAP ON KILL',
        timesTriggered: 0,
        creatures: ['Bugbear Berserker'],
      },
      rider_goblin: {
        name: 'RIDER',
        timesTriggered: 0,
        creaturesDeployed: 0,
        creatures: ['Goblin Wolf Rider'],
      },
      regenerate: {
        name: 'REGENERATE',
        timesTriggered: 0,
        hpHealed: 0,
        creatures: ['Feral Troll'],
      },
      flying_goblin: {
        name: 'FLYING',
        timesTriggered: 0,
        terrainIgnored: 0,
        creatures: ['Horned Devil'],
      },
      reach: {
        name: 'REACH 2',
        timesTriggered: 0,
        attacksFromDistance: 0,
        creatures: ['Horned Devil'],
      },
    },
  },
...
function trackCreatureAbility(factionKey, abilityKey, detail = {}) {
  ...
  switch (abilityKey) {
    ...
    case 'reach_2':
      ...
      if (detail.triggered) {
        ability.timesTriggered++
        if (detail.attackDistance)
          ability.attacksFromDistance = (ability.attacksFromDistance || 0) + 1
      }
...
function processAttackQueue(attackIntentions, gameState, currentPlayerId) {
  ...
  for (const intention of attackIntentions) {
    ...
    const defenderAI = new SimpleAI(
      gameState,
      defenderOwner,
      stats.creatureAbilityStats,
      defenderDifficulty
    )
    ...
    if (attackResult.success) {
      ...
      if (
        targetInfo.attackType === 'melee' &&
        gameState.hasFlanking &&
        gameState.hasFlanking(attackerInstance)
      ) {
        const potentialFlankingBonus = gameState.getFlankingBonus
        ...
        if (potentialFlankingBonus > 0) {
          ...
          if (useFlanking) {
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [FLANKING] ${attackerInstance.creature.name} dealt +${potentialFlankingBonus} bonus damage (ally adjacent to target)`
              )
            }
          } else {
            trackCreatureAbility('cormyr', 'flanking', {
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [FLANKING] ${attackerInstance.creature.name} declined FLANKING bonus (${attackerDifficulty} AI)`
              )
            }
          }
        }
      ...
      if (gameState.hasFlashingBlades && gameState.hasFlashingBlades(attackerInstance)) {
        ...
        if (flashingBladesTargets.length > 0) {
          ...
          if (useFlashingBlades) {
            ...
            const target =
              flashingBladesTargets[Math.floor(Math.random() * flashingBladesTargets.length)]
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [FLASHING BLADES] ${attackerInstance.creature.name} attacks ${target.creature.name} for ${splashDamage} splash damage!`
              )
            }
          } else {
      ...
      if (gameState.hasHiddenBlade && gameState.hasHiddenBlade(attackerInstance)) {
        ...
        if (hiddenBladeTargets.length > 0) {
          ...
          if (useHiddenBlade) {
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [HIDDEN BLADE] ${attackerInstance.creature.name} strikes ${target.creature.name} for ${hiddenBladeDamage} damage!`
              )
            }
          } else {
      ...
      if (gameState.hasConfusionGaze && gameState.hasConfusionGaze(attackerInstance)) {
        ...
        if (confusionGazeTargets.length > 0) {
          ...
          if (useConfusionGaze) {
            // Pick a random target from available targets
            const target =
              confusionGazeTargets[Math.floor(Math.random() * confusionGazeTargets.length)]
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [CONFUSION GAZE] ${attackerInstance.creature.name} slides and strikes ${target.creature.name} for ${confusionGazeDamage} damage!`
              )
            }
          } else {
        ...
        if (targetSurvived && damageDealt > 0) {
          ...
          if (slamTargets.length > 0) {
            ...
            if (useSlam) {
              ...
              if (CONFIG.VERBOSE_LOGGING) {
                console.log(
                  `  [SLAM] ${attackerInstance.creature.name} slams ${defenderInstance.creature.name} to (${randomTile.x}, ${randomTile.y})!`
                )
              }
            } else {
              trackCreatureAbility('cormyr', 'slam', {
      ...
      if (
        targetInfo.attackType === 'ranged' &&
        gameState.hasAcidBreath &&
        gameState.hasAcidBreath(attackerInstance)
      ) {
        ...
        if (splashTargets.length > 0) {
          ...
          if (useAcidBreath) {
            ...
            for (const splashTarget of splashTargets) {
              ...
              if (CONFIG.VERBOSE_LOGGING) {
                console.log(
                  `  [ACID BREATH] ${attackerInstance.creature.name} deals ${actualDamage} splash damage to ${splashTarget.creature.name}!`
                )
              }
            }
      ...
      if (
        targetInfo.attackType === 'ranged' &&
        gameState.hasExplosiveBolts &&
        gameState.hasExplosiveBolts(attackerInstance)
      ) {
        const splashTargets = gameState.getRangedSplashTargets
          ? gameState.getRangedSplashTargets(attackerInstance, defenderInstance.position)
          : []
        ...
        if (splashTargets.length > 0) {
          ...
          if (useExplosiveBolts) {
            ...
            for (const splashTarget of splashTargets) {
              ...
              if (CONFIG.VERBOSE_LOGGING) {
                console.log(
                  `  [EXPLOSIVE BOLTS] ${attackerInstance.creature.name} deals ${actualDamage} splash damage to ${splashTarget.creature.name}!`
                )
              }
            }
      ...
      if (
        targetInfo.attackType === 'melee' &&
        gameState.hasTapOnHit &&
        gameState.hasTapOnHit(attackerInstance)
      ) {
        ...
        if (attackResult.tapOnHitTriggered && attackResult.tapOnHitData) {
          ...
          if (tapData.alreadyTapped) {
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [TAP ON HIT] ${attackerInstance.creature.name} hit ${defenderInstance.creature.name} but already tapped`
              )
            }
          } else {
            ...
            if (CONFIG.VERBOSE_LOGGING) {
              console.log(
                `  [TAP ON HIT] ${attackerInstance.creature.name} tapped ${defenderInstance.creature.name}`
              )
            }
          }
          ...
          if (CONFIG.VERBOSE_LOGGING) {
            console.log(
              `  [TAP ON HIT] ${attackerInstance.creature.name} killed ${defenderInstance.creature.name} - no tap (destroyed)`
            )
          }
        }
      }
      ...
      switch (action.type) {
        case 'deploy':
          ...
          if (action.creatureTypes && action.creatureTypes.includes('Spider')) {
            ...
            const priestess =
              gameState.hasSummonSpider && gameState.hasSummonSpider(currentPlayerId)
            if (priestess) {
          ...
          if (gameState.hasBurrow) {
            ...
            if (movedCreature && gameState.hasBurrow(movedCreature)) {
              ...
              const mountainTilesInPath = path.filter((pos) => {
                ...
                return (
                  tile && (tile.terrain === 'MOUNTAIN' || tile.terrain === TerrainTypes.MOUNTAIN)
                )
              }).length
              ...
              if (mountainTilesInPath > 0) {
                ...
                if (CONFIG.VERBOSE_LOGGING) {
                  console.log(
                    `  [BURROW] ${movedCreature.creature.name} burrowed through ${mountainTilesInPath} mountain tile(s)!`
                  )
                }
              } else {
...
function printResults() {
  ...
  Object.entries(stats.difficultyStats).forEach(([difficulty, data]) => {
    if (data.gamesPlayed > 0) {
      const winRate =
        data.gamesPlayed > 0 ? ((data.wins / data.gamesPlayed) * 100).toFixed(1) : '0.0'
      console.log(`  ${difficulty.toUpperCase()}:`)
      console.log(`    Games Played:              ${String(data.gamesPlayed).padStart(6)}`)
      console.log(
        `    Wins:                      ${String(data.wins).padStart(6)}    (${winRate}%)`
      )
      console.log(`    Immediate Cards Used:      ${String(data.immediateCardsUsed).padStart(6)}`)
      console.log(
        `    Creature Abilities Used:   ${String(data.creatureAbilitiesUsed).padStart(6)}`
      )
    }
  })
  ...
  if (CONFIG.TRACK_CREATURE_ABILITIES) {
    ...
    for (const factionKey of factionKeys) {
      ...
      const hasActivity = Object.values(factionAbilities).some(
        (a) => a.timesTriggered > 0 || a.timesOffered > 0
      )
      if (hasActivity) {
        ...
        for (const [abilityKey, abilityData] of Object.entries(factionAbilities)) {
          if (abilityData.timesTriggered > 0 || abilityData.timesOffered > 0) {
            ...
            if (abilityKey === 'flashing_blades') {
              ...
              console.log(
                `      Times Offered:           ${String(abilityData.timesOffered).padStart(6)}    (${avgPerGame(abilityData.timesOffered)}/game)`
              )
              console.log(
                `      Times Triggered:         ${String(abilityData.timesTriggered).padStart(6)}    (${avgPerGame(abilityData.timesTriggered)}/game)`
              )
              console.log(
                `      Times Declined:          ${String(abilityData.timesDeclined).padStart(6)}    (${avgPerGame(abilityData.timesDeclined)}/game)`
              )
              console.log(
                `      Splash Damage Dealt:     ${String(abilityData.splashDamageDealt).padStart(6)}    (${avgPerGame(abilityData.splashDamageDealt)}/game)`
              )
              // Calculate usage rate when offered
              if (abilityData.timesOffered > 0) {
                const usageRate = (
                  (abilityData.timesTriggered / abilityData.timesOffered) *
                  100
                ).toFixed(1)
              ...
              console.log(
                `      Damage Dealt:            ${String(abilityData.damageDealt).padStart(6)}    (${avgPerGame(abilityData.damageDealt)}/game)`
              )
              // Calculate usage rate when offered
              if (abilityData.timesOffered > 0) {
                const usageRate = (
              ...
              console.log(
                `      Enemies Slid:            ${String(abilityData.enemiesSlid).padStart(6)}    (${avgPerGame(abilityData.enemiesSlid)}/game)`
              )
              console.log(
                `      Damage Dealt:            ${String(abilityData.damageDealt).padStart(6)}    (${avgPerGame(abilityData.damageDealt)}/game)`
              )
              ...
              console.log(
                `      Orcs Deployed:           ${String(abilityData.orcsDeployed).padStart(6)}    (${avgPerGame(abilityData.orcsDeployed)}/game)`
              )
              console.log(
                `      Leadership Gained:       ${String(abilityData.leadershipGained).padStart(6)}    (${avgPerGame(abilityData.leadershipGained)}/game)`
              )
              // Calculate usage rate when offered
              if (abilityData.timesOffered > 0) {
                const usageRate = (
              ...
              console.log(
                `    ${abilityData.name}: ${abilityData.timesTriggered} triggers (${avgPerGame(abilityData.timesTriggered)}/game)`
              )
            }
          }
        }
...
function resetStats() {
  ...
  Object.keys(stats.difficultyStats).forEach((key) => {
    stats.difficultyStats[key] = {
      gamesPlayed: 0,
      wins: 0,
      immediateCardsUsed: 0,
      creatureAbilitiesUsed: 0,
    }
  })
...
function validateAbilityUsageRates(difficulty) {
  ...
  if (flashingBlades.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `FLASHING BLADES: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (hiddenBlade.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `HIDDEN BLADE: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (shadowStalker.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `SHADOW STALKER: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (confusionGaze.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `CONFUSION GAZE: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (acidBreath.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `ACID BREATH: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (explosiveBolts.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `EXPLOSIVE BOLTS: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
  ...
  if (chieftainCall.timesOffered > 0) {
    ...
    if (Math.abs(actualRate - expectedRate) > tolerance) {
      issues.push(
        `CHIEFTAIN CALL: Expected ~${(expectedRate * 100).toFixed(0)}% usage, got ${(actualRate * 100).toFixed(1)}%`
      )
    }
  }
...
console.log(
  `Tracking: Commander Abilities + Creature Abilities (${CONFIG.TRACK_CREATURE_ABILITIES ? 'enabled' : 'disabled'})`
)
console.log('')
...
if (CONFIG.AI_DIFFICULTY === 'all') {
  ...
  for (const difficulty of difficulties) {
    ...
    const rate =
      fb.timesOffered > 0 ? ((fb.timesTriggered / fb.timesOffered) * 100).toFixed(1) : '0.0'
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)}          ${String(fb.timesOffered).padStart(6)}    ${String(fb.timesTriggered).padStart(6)}     ${String(fb.timesDeclined).padStart(6)}    ${rate}%`
    )
  }
  ...
  for (const difficulty of difficulties) {
    ...
    const rate =
      hb.timesOffered > 0 ? ((hb.timesTriggered / hb.timesOffered) * 100).toFixed(1) : '0.0'
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)}          ${String(hb.timesOffered).padStart(6)}    ${String(hb.timesTriggered).padStart(6)}     ${String(hb.timesDeclined).padStart(6)}    ${rate}%`
    )
  }
  ...
  for (const difficulty of difficulties) {
    ...
    const rate =
      ss.timesOffered > 0 ? ((ss.timesTriggered / ss.timesOffered) * 100).toFixed(1) : '0.0'
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)}          ${String(ss.timesOffered).padStart(6)}    ${String(ss.timesTriggered).padStart(6)}     ${String(ss.timesDeclined).padStart(6)}    ${rate}%`
    )
  }
  ...
  for (const difficulty of difficulties) {
    ...
    const rate =
      cg.timesOffered > 0 ? ((cg.timesTriggered / cg.timesOffered) * 100).toFixed(1) : '0.0'
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)}          ${String(cg.timesOffered).padStart(6)}    ${String(cg.timesTriggered).padStart(6)}     ${String(cg.timesDeclined).padStart(6)}    ${rate}%`
    )
  }
  ...
  for (const difficulty of difficulties) {
    ...
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)} Expected: ${(expectedRate * 100).toFixed(0)}%   Actual: ${(actualRate * 100).toFixed(1)}%  ${status}`
    )
  }
  ...
  for (const difficulty of difficulties) {
    ...
    const rate =
      cc.timesOffered > 0 ? ((cc.timesTriggered / cc.timesOffered) * 100).toFixed(1) : '0.0'
    console.log(
      `  ${difficulty.toUpperCase().padEnd(8)}          ${String(cc.timesOffered).padStart(6)}    ${String(cc.timesTriggered).padStart(6)}     ${String(cc.timesDeclined).padStart(6)}    ${rate}%`
    )
  }
